...
namespace eae6320
{
    namespace Graphics
    {
		cResult View::InitializeViews(const sInitializationParameters& i_initializationParameters)
        {
            cResult result = Results::Success;
#ifdef EAE6320_PLATFORM_D3D
			const uint16_t i_resolutionWidth = i_initializationParameters.resolutionWidth;
			const uint16_t i_resolutionHeight = i_initializationParameters.resolutionHeight;
			ID3D11Texture2D* backBuffer = nullptr;
			ID3D11Texture2D* depthBuffer = nullptr;
			cScopeGuard scopeGuard([&backBuffer, &depthBuffer]
				{
			...
			auto& g_context = Graphics::sContext::g_context;
			...
			// meaning a texture that the GPU can render to)
			{
				// Get the back buffer from the swap chain
				{
					...
					if (FAILED(d3dResult))
					{
						...
						Logging::OutputError("Direct3D failed to get the back buffer from the swap chain (HRESULT %#010x)", d3dResult);
						return result;
					}
				}
				// Create the view
				{
					...
					if (FAILED(d3dResult))
					{
						...
						Logging::OutputError("Direct3D failed to create the render target view (HRESULT %#010x)", d3dResult);
						return result;
					}
				}
			}
			// Create a depth/stencil buffer and a view of it
			{
				// Unlike the back buffer no depth/stencil buffer exists until and unless it is explicitly created
				{
					const auto textureDescription = [i_resolutionWidth, i_resolutionHeight]
						...
						if (FAILED(d3dResult))
						{
							...
							Logging::OutputError("Direct3D failed to create the depth buffer resource (HRESULT %#010x)", d3dResult);
							return result;
						}
				}
				// Create the view
				{
					...
					if (FAILED(d3dResult))
					{
						...
						Logging::OutputError("Direct3D failed to create the depth stencil view (HRESULT %#010x)", d3dResult);
						return result;
					}
				}
			}
